...
export function deriveMockBlockHash(payloadAttributes: PayloadAttributesV3): string {
  return stableHex(
    [
      'block',
...
function deriveMockPayloadId(payloadAttributes: PayloadAttributesV3): string {
  return stableHex(['payload', payloadAttributes.slot, payloadAttributes.builderId].join(':')).slice(0, 34)
}

function createMockExecutionPayload(payloadAttributes: PayloadAttributesV3): ExecutionPayloadV3 {
...
function createMockBlobsBundle(blockHash: string): BlobsBundle {
  return {
...
function createEnvelopeObservation(envelope: ExecutionPayloadEnvelope): EnvelopeObservation {
  return {
...
export class PayloadStore {
  private readonly buildsByPayloadId = new Map<string, BuildRecord>()
  private readonly buildsByBlockHash = new Map<string, BuildRecord>()

  createBuild(payloadAttributes: PayloadAttributesV3, createdAtMs: number): BuildRecord {
  ...
  getPayload(payloadId: string, fetchedAtMs: number): { executionPayload: ExecutionPayloadV3; blobsBundle: BlobsBundle } {
    const record = this.requireBuild(payloadId)
    record.fetchedAtMs = fetchedAtMs
    if (record.state === 'BUILDING') record.state = 'FETCHED'
  ...
  markWithheld(payloadId: string): BuildRecord {
    const record = this.requireBuild(payloadId)
    if (record.state !== 'EQUIVOCATED') record.state = 'WITHHELD'
    return clone(record)
  }

  recordEnvelope(payloadId: string, envelope: ExecutionPayloadEnvelope): BuildRecord {
    const record = this.requireBuild(payloadId)
  ...
  validateNewPayload(params: NewPayloadV3Params, validatedAtMs: number): NewPayloadV3Result {
    ...
    const bundleMatches =
      params.blobsBundle.commitments[0] === record.blobsBundle.commitments[0] &&
  ...
  snapshotByPayloadId(payloadId: string): BuildRecord | undefined {
    const record = this.buildsByPayloadId.get(payloadId)
    return record ? clone(record) : undefined
  }

  snapshotByBlockHash(blockHash: string): BuildRecord | undefined {
    const record = this.buildsByBlockHash.get(blockHash)
    return record ? clone(record) : undefined
  }

  private requireBuild(payloadId: string): BuildRecord {
    const record = this.buildsByPayloadId.get(payloadId)
    if (!record) throw new Error(`unknown payloadId ${payloadId}`)
    return record
  }
}
